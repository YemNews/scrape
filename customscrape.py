...
def customparser(text):
    soup = BeautifulSoup(text, 'xml')

    feed_info = {
        'title': soup.channel.title.text,
        'link': soup.channel.link.text,
        'description': soup.channel.description.text
    }  
    items = []
    ...
    # Create a dictionary with feed info and items
    rss_dict = {
        'feed': feed_info,
        'entries': items
    }
